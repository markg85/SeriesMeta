...
async function getSeries(series) {
  ...
  let matchResult = seriesLower.match(/^(t{2}\d{7,8})$/gi);
    ...
    if (fromImdbID) {
      try {
        ...
        return Promise.reject(
          `Failed to get data by IMDB ID for: ${seriesLower}`
        );
      }
    } else {
          await axiosReq(
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let episode of data._embedded.episodes) {
      episode.datetime = Moment(episode.airstamp).tz(
  ...
  latestEpisode: async (series) => {
    try {
      ...
      if (index < 0) {
        ...
        let obj = await module.exports.episodesByDate(
          series,
          Moment(episodes[index].datetime).format("YYYY-MM-DD")
        );
